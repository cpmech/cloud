import { maybeCopySimple } from '../maybeCopySimple';
...
describe('maybeCopySimple', () => {
  it('the copy should be equal to the original object', () => {
    ...
    maybeCopySimple(res1, obj1);
    maybeCopySimple(res2, obj2);
    maybeCopySimple(res3, obj3);
  ...
  it('should not copy undefined or null values', () => {
    const res = {
      hello: '<erase>',
      world: '<erase too>',
      data: { nested: { label: '<replace>', values: { a: 1000, b: 666 } } },
      array: ['', '123'],
    };
    maybeCopySimple(res, {
      hello: '',
      world: '',
      data: { nested: { label: 'just testing', values: { a: 0, b: -2000 } } },
      array: ['a', 'b', 'c'],
    });
    expect(res).toEqual({
      hello: '',
  ...
  it('should copy empty strings and zero numbers', () => {
    const res = {
      hello: '<erase>',
      world: '<erase too>',
      data: { nested: { label: '<replace>', values: { a: 1000, b: 666 } } },
    ...
    maybeCopySimple(res, {
      hello: undefined,
      world: null,
      data: { nested: { label: 'just testing', values: { a: 0 } } },
    });
    expect(res).toEqual({
      hello: '<erase>',
      world: '<erase too>',
      data: { nested: { label: 'just testing', values: { a: 0, b: 666 } } },
      array: ['', '123'],
    });
  });
});
