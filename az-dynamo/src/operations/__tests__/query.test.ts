...
describe('query operation', () => {
  ...
  it('should query using "prefix"', async () => {
    const r1 = await query(tableName, 'itemId', 'get', 'aspect', 'A', undefined, 'prefix');
    const r2 = await query(tableName, 'itemId', 'get', 'aspect', 'L', undefined, 'prefix');
    expect(r1).toEqual([data1]);
    expect(r2).toEqual([data2]);
  });

  it('should query the index', async () => {
    const res = await query(tableName, 'aspect', 'NAME', undefined, undefined, tableIndex);
